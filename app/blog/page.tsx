...
export const metadata: Metadata = { title: "Blog de Yu-Gi-Oh! Forbidden Memories | FM Codex", description: "Posts publicados no WordPress sobre Forbidden Memories." };

export default async function BlogPage() {
  const posts = await getPublishedPosts();
  const featured = posts[0];
  const schema = { "@context": "https://schema.org", "@type": "Blog", name: "Blog FM Codex", blogPost: posts.map((post) => ({ "@type": "BlogPosting", headline: post.title.rendered, image: post.featuredImage, datePublished: post.date, url: `/blog/${post.slug}/` })) };
