...
test("renders the Yu-Gi-Oh! Forbidden Memories home with its primary discovery paths", async () => {
  ...
  const html = await response.text();
  assert.match(html, /<title>Yu-Gi-Oh! Forbidden Memories \(PS1\): Cartas, Fusões, Drops e Guias<\/title>/i);
  assert.match(html, /name="description" content="Yu-Gi-Oh! Forbidden Memories \(PS1\) em português:/i);
  assert.equal((html.match(/<h1\b/gi) || []).length, 1);
  assert.match(html, /<h1>Yu-Gi-Oh! Forbidden Memories<br\/><span>cartas, fusões, drops e passwords<\/span><\/h1>/i);
  const intro = html.match(/<section class="home-intro"[\s\S]*?<\/section>/i)?.[0] || "";
  const introWordCount = intro.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim().split(" ").filter(Boolean).length;
  assert.ok(introWordCount >= 150 && introWordCount <= 300, `intro has ${introWordCount} words`);
  assert.match(html, /O que é Yu-Gi-Oh! Forbidden Memories\?/i);
  assert.match(html, /Como fazer fusões em Yu-Gi-Oh! Forbidden Memories\?/i);
